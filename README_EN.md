...
<p>
  ...
  <b>Go from ideas to presentations in minutes—no tedious formatting, just conversational edits. Step into the era of "Vibe PPT".</b>
</p>
<p>
...
Have you ever found yourself in this dilemma: you have a presentation tomorrow, but the slides are still blank; your mind is full of brilliant ideas, yet all your passion is drained by tedious layout and design?
...
- 3️⃣ Similar visual output with serious homogenization
...
However, the emergence of the nano banana🍌 model changed everything. I tried using 🍌pro for PPT page generation and found that the results were exceptional in terms of quality, aesthetics, and consistency. Furthermore, it can accurately render almost all text requested in the prompts while following the style of reference images. So, why not build a native "Vibe PPT" application based on 🍌pro?
...
2. **PPT Professionals**: Gain design inspiration quickly by referencing AI-generated layouts and combinations of text and visual elements.
3. **Educators**: Quickly convert teaching content into visualized lesson plan PPTs to enhance classroom effectiveness.
4. **Students**: Complete assignment presentations quickly, focusing energy on content rather than layout and formatting.
...
Supports three startup modes: **Idea**, **Outline**, and **Page Description**, catering to different creative habits.
...
- **Local Redrawing**: Perform verbal-style modifications on unsatisfactory areas (e.g., "change this chart to a pie chart").
...
- Supports configuration of multiple speaking styles, languages, and voice personas
...
| Re-editing | Prompt-based modifications | **Box-selection editing + Verbal editing** |
...
- [2026-04-25]: Supports account linking via official OpenAI OAuth. Once linked, Codex can be used directly as a text/image generation provider without manually entering an API Key (Based on official OpenAI OAuth PKCE authorization flow, non-reverse engineered).
- [2026-04-25]: Supports saving custom text style description templates, allowing for naming, color tagging, and persistent reuse without needing to re-enter details every time.
- [2026-04-23]: Added support for the gpt-image-2 model. Editable background effects during export have also been improved due to model upgrades (Select "Generative Acquisition" in Settings - Export Options - Background Acquisition).
...
- [2026-02-09]: New Features and Optimizations
  * New Features
    * Supports pasting images in the homepage, outline, and description cards for immediate recognition with an improved interactive experience.
...
- [2026-01-04]: v0.4.0 Release: Comprehensive upgrade for editable PPTX export:
  * Maximum restoration of font size, color, bolding, and other styles for text in images.
...
| 🔄 In Progress | Support for exporting editable .pptx with multi-layered, precise image masking |
| 🔄 In Progress | Web search |
| 🔄 In Progress | Agent mode |
| ✅ Completed | TTS narration video export (CN/EN/JP multi-voice, subtitles, Ken Burns effect) |
...
1. One-click deploy and launch this application via Rainyun (High bandwidth, ideal for HD image generation and downloading. New users get a 15-day free trial)
...
> **The LLM interfaces in this project follow the AIHubMix platform format. It is recommended to use [AIHubMix (click here to access directly)](https://aihubmix.com/?aff=17EC) to obtain an API key and reduce migration costs.**<br>
> **Friendly tip: The Google nano banana pro model interface has higher costs; please be mindful of usage.**
```env
...
**Use the new version of the editable export configuration method to get better editable export results**: You need to obtain an API KEY from the [Baidu AI Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) (click here to enter) and fill it in the `BAIDU_API_KEY` field in the `.env` file (there is sufficient free usage quota). See the instructions in https://github.com/Anionex/banana-slides/issues/121 for details.
...
# Starting with Pre-built Images (No building from scratch required)
...
> [!TIP]
> If you encounter network issues, you can uncomment the mirror source configurations in the `.env` file, then run the startup command again:
> ```env
...
# Then, as described above, open and edit the `.env` file to configure your API key

Please provide the Chinese Markdown content you would like me to translate. The "Original content" section in your request currently appears to be empty. 

Once you provide the content, I will translate it into English while strictly following your requirements (keeping formatting, HTML tags, and URLs unchanged, and using standard technical English terminology).
...
Feel free to suggest new features or provide feedback; I will also answer questions ~~casually~~.

<img width="312" alt="image" src="https://github.com/user-attachments/assets/f96ec821-20d8-469f-9b40-ddb7d4db8df2" />


Feel free to follow the author's social media, where I will share updates about this project and AI:
