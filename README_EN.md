...
<b>From idea to presentation in minutes—no tedious formatting, verbal modification requests, moving towards true "Vibe PPT"</b>
...
Have you ever found yourself in this dilemma: the presentation is due tomorrow, but your PPT is still a blank slate; you have countless brilliant ideas in your head, but all your passion is drained by tedious layout and design?

We long to quickly create presentations that are both professional and well-designed. Although traditional AI PPT generation apps generally meet the requirement of "speed," they still suffer from the following issues:
...
- 5️⃣ Disconnected text and image layout, resulting in poor design quality.
...
However, the emergence of the nano banana🍌 model has changed everything. I tried using 🍌pro to generate PPT pages and found that the results were excellent in terms of quality, aesthetics, and consistency. Moreover, it can accurately render almost all text requested in the prompts and follow the style of reference images. So, why not build a native "Vibe PPT" application based on 🍌pro?

## 👨‍💻 Use Cases

1. **Novices**: Quickly generate beautiful PPTs with zero barrier and no design experience required, reducing the hassle of selecting templates.
...
5. **Workplace Professionals**: Quickly visualize business proposals and product introductions with rapid adaptation for multiple scenarios.
...
- **Natural Language Editing**: Supports modifying outlines or descriptions using "Vibes" (e.g., "Change the third page to a case study"), with AI responding and adjusting in real-time.
...
- **Multi-format Support**: Upload PDF, Docx, MD, Txt, and other files for automatic content parsing.
...
- **Local Inpainting**: Perform conversational edits on specific areas (e.g., "change this chart to a pie chart").
...
- **Perfect Fit**: Default 16:9 ratio, no manual layout adjustments needed, ready for presentation.
...
| Re-editing | Prompt-based modification | **Selection-based + Natural language editing** |
| Asset Addition | Cannot add after generation | **Add freely after generation** |
| Export Formats | Supports PDF, (non-editable image) pptx | **Export to PDF, (image or editable) pptx** |
...
- [2026-04-25]: Asset Toolbox launched — Added three new modes: full image editing, selection editing (overlay/replace), and smart erasure, built upon the original asset generation. Unified entry for a one-stop operation.
- [2026-04-25]: Supports account binding via official OpenAI OAuth. Once bound, Codex can be used directly as a text/image generation provider without manually entering an API Key (based on official OpenAI OAuth PKCE authorization flow, non-reverse-engineered).
...
- [2026-02-09] : New Features and Optimizations
  ...
  * Fixes & Experience Optimizations
    ...
    * Optimized format example prompts, HTTP error message copy, modal closing experience, cleaned up old project localStorage, and removed redundant prompts during first-time project creation.
    * Several other optimizations and fixes.
- [2026-01-04] : v0.4.0 Released: Major upgrade to editable PPTX export:
  * Maximally restores styles like font size, color, and bolding for text within images;
  ...
  * **See https://github.com/Anionex/banana-slides/issues/121 for detailed results and usage instructions.**

- [2025-12-27] : Added support for image-less template mode and high-quality text presets. PPT page styles can now be controlled through pure text descriptions.
...
| ✅ Completed | Vibe verbal editing for selected areas on single slides |
| ✅ Completed | Asset module: Asset generation, uploading, etc. |
| ✅ Completed | Support uploading and parsing of various file formats |
...
1. One-click deploy and launch this application via RainYun (High bandwidth, suitable for HD image generation and downloads. New users enjoy a 15-day free trial)
...
**(Optional, can also be configured via the UI after startup)** Edit the `.env` file to configure necessary environment variables:
> **The LLM API in this project follows the AIHubMix platform standard. It is recommended to use [AIHubMix (click here to visit)](https://aihubmix.com/?aff=17EC) to obtain an API key to reduce migration costs.**<br>
...
# Vendor API Keys (Only configure the providers you want to use)
...
**Use the new version of the editable export configuration method for better editable export results**: You need to obtain an API KEY from the [Baidu AI Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) (click here to enter), and fill it into the `BAIDU_API_KEY` field in the `.env` file (sufficient free quota is available). See the instructions in https://github.com/Anionex/banana-slides/issues/121 for details.


<details>
  <summary>📒 Vertex AI Configuration Guide (For GCP Users)</summary>

Google Cloud Vertex AI allows calling Gemini models through GCP service accounts, and new users can use free trial credits. Configuration steps:
...
> [!TIP]
> If you encounter network issues, uncomment the mirror source configurations in the `.env` file, then rerun the startup command:
> ```env
...
- (Optional) [LibreOffice](https://www.libreoffice.org/) - Required when uploading PPTX files using the "PPT Refurbishment" feature, used to convert PPTX to PDF. **It is recommended to convert PPTX to PDF locally before uploading**. Reason: LibreOffice may cause layout misalignment during server-side rendering due to missing fonts (such as Microsoft YaHei, Calibri, etc.) and cannot fully reproduce certain special effects. Uploading PDF files does not require LibreOffice. Docker users who still need to support PPTX uploads within the container can execute:
  ...
  ```
  > Note: LibreOffice installed via this method will be lost after the container is recreated and will need to be reinstalled.
...
> **The LLM interfaces in this project follow the AIHubMix platform format. It is recommended to use [AIHubMix](https://aihubmix.com/?aff=17EC) to obtain API keys and reduce migration costs.** 
```env
...
# 🚀 One-click Deployment and Usage Guide

Welcome to this project! This guide will help you quickly complete environment configuration, project deployment, and daily maintenance.

## 🛠 Environment Preparation

### 1. Basic Software
- **Docker & Docker Compose** (Recommended version: v20.10+)
- **Node.js** (LTS version)
- **Python 3.9+** (If local execution is required)
...
Copy the template file and modify the configuration according to your actual situation:
```bash
cp .env.example .env
```
...
Open your browser to this address to use the application.
...
1. **Text on the generated page is garbled or blurry**
    - You can select a higher resolution output (the OpenAI format may not support adjusting the resolution; using the Gemini format is recommended). According to tests, upgrading from 1k to 2k resolution before generating the page significantly improves text rendering quality.
...
3. **Does it support the Gemini API Key free tier?**
...
4. **Receiving a 503 error or Retry Error during content generation**
    - You can view the Docker backend logs using the commands in the README to locate the detailed error for the 503 issue. This is generally caused by incorrect model configuration.

5. **Why is the API Key not taking effect after being set in .env?**
    - You need to restart the Docker container after editing `.env` during runtime to apply the changes.
    - If you have configured parameters on the web settings page, they will override the parameters in `.env`. You can revert to the `.env` settings by using "Restore Default Settings."
...
If you require a Commercial License (e.g., you wish to use it as closed-source, deliver via private deployment, integrate this project into closed-source products, or provide services without disclosing the corresponding source code), please contact the author: davidyang042@gmail.com
...
<details>
  ...
  "Aggregating global multi-model API service providers. Enjoy secure, stable services and access the world's latest models within 72 hours at a lower price."
</details>
