...
def upgrade() -> None:
    ...
    # Older deployments created this table outside Alembic before recording
    # this revision. Treat the migration as idempotent so those databases can
    # join the current migration graph safely.
    if 'access_codes' not in inspector.get_table_names():
        op.create_table(
            'access_codes',
    ...
    existing_indexes = {index['name'] for index in inspector.get_indexes('access_codes')}
    if 'ix_access_codes_code_hash' not in existing_indexes:
